...
with open(pdb_path, 'r') as PDB:
    for line in PDB:
        ...
        if line.startswith("ATOM") or line.startswith("HETATM"):
            ...
            # AF2/AF3 tokenize a PTM / non-standard residue atom-by-atom, so each of its non-CA atoms
            # is its own token in the plddt/pae arrays (append 0). Boltz-2, however, keeps a modified
            # residue that sits inside a polymer chain (numeric label_seq_id, e.g. PCA pyroglutamate) as
            # a SINGLE token represented by its CA/C1 atom; its remaining atoms are NOT separate tokens.
            # Appending per-atom 0s for Boltz over-counts the mask (e.g. PCA: 1 token but 8 atoms) and
            # makes token_array longer than plddt/pae -> IndexError when slicing. True ligands
            # (label_seq_id == '.') already return None above and stay per-atom for both pipelines.
